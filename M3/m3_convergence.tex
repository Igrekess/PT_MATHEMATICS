...
\emph{symmetry parameter}
$\alpha_k = n_{12}(k) / [n_{12}(k) + n_{21}(k)]$---the conditional
fraction of $1 \to 2$ transitions among all off-diagonal transitions
at sieve level $k$---actually converge to $s = 1/2$?  The same symbol
$\alpha_k$ is used consistently throughout this chapter with this
single definition; see Definition~\ref{def:alpha_k} below for the
formal statement.  At finite sieve level $k$, $\alpha_k$ oscillates
around $1/2$ with an amplitude that empirically decreases. The
question is whether this decrease can be guaranteed algebraically.

The answer comes in layers. At the finite level, exact recurrences
together with a polynomial factorization give a completely
...
\end{align}
\begin{definition}[Symmetry parameter $\alpha_k$]
\label{def:alpha_k}
The \emph{symmetry parameter} at sieve level~$k$ is
\begin{equation}
  \alpha_k \;=\; \frac{n_{12}(k)}{n_{12}(k) + n_{21}(k)},
  \label{eq:alpha_k_def}
\end{equation}
the conditional probability that an off-diagonal mod-$3$ transition
is of type $1 \to 2$ rather than $2 \to 1$.  This single definition
is used throughout the article.  Note that $\alpha_k$ is \emph{not}
the self-transition fraction $T_{00}$
(see Remark~\ref{rem:alpha_vs_T00}).
\end{definition}

\begin{remark}[$\alpha_k$ vs self-transition fraction $T_{00}$]
\label{rem:alpha_vs_T00}
Two distinct quantities are sometimes confused in discussions of~T4.
\begin{itemize}[nosep]
\item \textbf{The symmetry parameter $\alpha_k$}
  (Definition~\ref{def:alpha_k}): a conditional probability among
  off-diagonal transitions. By Dirichlet's theorem on primes in
  arithmetic progressions, $n_{12} = n_{21}$ exactly at the
  primorial-sieve level, so $\alpha_k = 1/2$ is an \emph{exact
  identity} at every level $k \geq 2$. T4 states that the deviation
  from $1/2$ at the prime level decays to zero as $k \to \infty$.
\item \textbf{The self-transition fraction $T_{00}(N)$}: the empirical
  probability that two consecutive primes $\leq N$ share the same
  residue class mod~3. This quantity is \emph{not} zero at finite $N$
  (it is of order $1/\ln N$, reaching $\sim 0.40$ up to $N = 10^6$).
  It converges to zero only asymptotically (as $N \to \infty$).
  T1 states that $T_{00}$ vanishes at the \emph{sieve level}
  (6-rough integers), not at the prime level.
\end{itemize}
Both quantities approach zero asymptotically but through different
mechanisms. ``Self-transition fraction converges to zero'' means
$T_{00}(N) \to 0$, \emph{not} $\alpha_k \to 0$ (which would be
false: $\alpha_k \to 1/2$).
\end{remark}
...
The algebraic derivation is in the PT core notes;
the full data through $k = 11$ are in the T4 demonstration
document (companion monograph~\cite{SenezMonograph2026}).
\end{proof}
\end{thmbox}
...
\begin{tcolorbox}[colback=gray!5,colframe=gray!60,
  title=\textbf{Anticipated objections (Section~\ref{sec:fixed_point})}]
\label{box:m3_objections}
Two objections a reader is most likely to raise, with pointers to
their discussion.
\begin{itemize}[nosep,leftmargin=1.2em]
\item \emph{``$p = 2$ satisfies the activity criterion $\gamma_2 > 1/2$
  at $\mu = 15$ (numerically $\gamma_2 \approx 0.867$), so excluding
  it from the cascade sum is ad hoc.''}
  The exclusion rests on condition~\textbf{I1} in
  Definition~\ref{def:binary_infrastructure}: $T_2$ is the unique
  $1 \times 1$ transfer matrix among all primes, a property no odd
  prime shares.  The motivation is the algebraic singularity of
  $p = 2$, not the target value of $\mustar$; see the structural
  commitment discussion in the monograph~\cite{SenezMonograph2026}
  (Remarks on $p = 2$ in Chapter~8).
\item \emph{``$\mustar = 15$ is not unique: $\mu = 10$ and $\mu = 17$
  are also fixed points.''}
  Correct.  Theorem~\ref{thm:T5a} identifies $\mu = 10$ and $\mu = 17$
  as the \emph{raw} fixed points (over the full prime set); the
  reduced cascade equation (after the infrastructure quotient for
  $p = 2$) yields $\mustar = 15$ as its unique solution
  (Theorem~\ref{thm:T5b}).  The three values are organised as a
  cosmogonic sequence $\mu = 10 \to \mu = 17 \to \mustar = 15$
  (Remark~\ref{rem:cosmogonic}).
\end{itemize}
\end{tcolorbox}
...
\item \label{it:I2}
  ...
  activity threshold (the channel is structurally present, not ghost).
\item \label{it:I3}
...
% ============================================================
\subsection{Ghost primes and the inactive sector}
\label{sec:ch8_ghost}
...
and as \emph{ghost modes} in the vacuum polarization correction (R28).

\begin{idbox}{Ghost Prime Identity}
\label{id:ghost}
The ghost vacuum polarization correction is
\begin{equation}
  \delta_{\rm ghost} = -\gamma_3 \cdot \alphaEM \cdot \beta_{\rm ghost},
  \qquad
  \beta_{\rm ghost} = \sum_{p \in \{11,13\}} \sinp{p}\,\gamp{p}
  \approx 0.104,
...
The physical meaning is: ghost primes are not ``absent'' from the
...
\emph{structurally linked}: both descend from the same Koide--ghost
...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[dissgray, font=\small\itshape] at (5.5, 0.5) {ghost (counter-terms)};
\end{tikzpicture}
...
(blue circles) from the ghost primes (gray circles).}
...
of~$p = 2$, the anomalous dimension hierarchy, and the ghost sector.
...
$p = 11$ is a ghost, and the cascade converges to $\mustar = 15$.
...
At the stable fixed point $\mustar = 15$, the first two ghost
primes have anomalous dimensions:
\begin{equation}
  \gamp{11}(15) = 0.426, \qquad \gamp{13}(15) = 0.356.
  \label{eq:ghost_gamma_values}
\end{equation}
Both lie below $s = 1/2$.  The margin of the first ghost is
\begin{equation}
...
non-physical).  The ghost primes are therefore the \emph{price} of
...
\textbf{Hard core (unconditional):} T5 (three fixed points:
  ...
  ghost prime identity~\eqref{eq:ghost_correction};
...
\textbf{Validation:} J6 ($\mu_\alpha = 15.0397$ minimizes $\alpha$ error);
  anomalous dimension hierarchy exactly as computed; ghost correction
