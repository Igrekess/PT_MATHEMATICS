...
$\{2,3,5,7,11\}$        & $28$ & $\mustar = 6079$ & 53 & Dimensional explosion \\
$\{2,3,5,7,11,13\}$     & $41$ & $\mustar = 6079$ & 53 & Same explosion \\
$\{2,3,5,7,\ldots,19\}$ & $77$ & $\mustar = 6079$ & 53 & Same explosion \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Dimensional explosion} ($\mathcal{B} \supseteq
  \{2,3,5,7,11\}$): the iteration runs away to $\mustar = 6079$
  with 53~active primes.
\end{itemize}
...
in a runaway cascade.  The iteration stabilises only at
$\mustar = 6079$, where the first 53 odd primes are all
simultaneously active.  This is not a physical universe: it has
53~effective spatial dimensions.
...
$18$ & $\mustar = 6079$ & 53 active primes \\
\bottomrule
\end{tabular}
\end{center}

At $\mu_0 = 17$, $\gamp{11}(17) = 0.478 < 1/2$: the prime
$p = 11$ is a ghost, and the cascade converges.  At $\mu_0 = 18$,
$\gamp{11}(18) = 0.503 > 1/2$: the prime $p = 11$ crosses the
activity threshold, triggering the runaway cascade that terminates
at 53~dimensions.  The bifurcation is \emph{sharp}: a single unit
step in $\mu_0$ separates 3D stability from 53D catastrophe.
...
terminating at $\mustar = 6079$ with 53~active primes---a
53-dimensional effective space.  The ghost primes are therefore
the \emph{price} of three-dimensional stability: they are present
as perturbative corrections (NLO and NNLO counter-terms, see
...
\item For $\mathcal{B} \supseteq \{2,3,5,7,11\}$: $\mu_0 \geq 28$.
  ...
  $\gamp{13}(26) > 1/2$, the cascade continues upward.  The unique
  fixed point of this runaway branch is $\mustar = 6079$ (verified by
  exhaustive scan up to $\mu = 10^4$).
\end{enumerate}
Stability of $\mustar = 15$ follows from
Theorem~\ref{thm:stability} ($|F'(15)| < 1$).
The point $\mustar = 6079$ is formally a fixed point but is
\emph{not} accessible from $\mathcal{B} = \{2,3,5,7\}$; it requires
at least 11 to be present in the initial base.
\end{proof}
...
explode to $\mustar = 6079$ (53~active primes).  The prime $p = 7$ is
