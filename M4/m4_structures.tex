...
and the activation threshold formula in three regimes (ghost,
...
\noindent
A consolidated monograph~\cite{Senez2026monograph} unifies all five
articles with the physics and chemistry applications.
...
\begin{thmbox}{Decoherence Theorem (H)}
\label{thm:theorem_H}
...
the sieve channel.  The convergence rate is controlled by the
spectral gap $1 - |\lambda_2(T_3)|$ of the transfer matrix.
\end{thmbox}
...
\paragraph{Ghost primes.} Primes $p > p_K$ that lie below $\max\_\mathrm{gap}$
...
$d = n - 1$ grows faster than the number of ghost primes, so the
...
(the smallest active prime). The ghost primes ($p = 11, 13$) serve as
...
\epigraph{It from bit. Every ``it'' --- every particle, every field of
force, even the spacetime continuum itself --- derives its function,
its meaning, its very existence entirely from the apparatus-elicited
answers to yes-or-no questions, binary choices, bits.}{John A.\ Wheeler, ``Information, Physics, Quantum: The Search for Links'', in W.~Zurek (ed.), \textit{Complexity, Entropy, and the Physics of Information}, Addison-Wesley (1990)}
...
\textbf{MAIN RESULT:}
  ...
  \item AT formula in three regimes: ghost, construction, asymptotic
...
\begin{itemize}[nosep]
  ...
  \item Conditioning: residues modulo the active primes $\{3, 5, 7\}$
    and ghost primes $\{11, 13, \ldots\}$ already in the base.
\end{itemize}
...
$|B| = |B(p)| = \pi(p) - 4$,
the number of ghost conditioning levels (primes between 7 and~$p$, exclusive).
...
\subsubsection{Part B: ghost regime ($|B| \le 2$)}

\begin{thmbox}{Activation Threshold B}
\label{thm:AT_B}
For $p \in \{11, 13\}$ (ghost primes, $|B| \le 2$): $\mathrm{AT}(p) = 1$.
\end{thmbox}

\begin{proof}
The primes 11 and 13 are the first ghost primes ($\gamp{11} = 0.426$,
...
For $|B| \le 2$: only 1--2 ghost conditioning levels; the 3~CRT channels are
not all saturated.  Two structural directions survive:
$\mathrm{struct}(1) = 2 > 0$.

For $|B| \ge 3$: the $|B|$ ghost levels provide enough non-uniformity to
...
\label{thm:AT_C}
Under Conjecture~\ref{hyp:gen_pos}, for non-ghost shells of cycle~1
($3 \le |B| \le 8$):
\[
...
phantom surplus to overcome is $|B| - 2$ (the $|B|$ ghost levels minus the 2
...
\midrule
11 & 1 & ghost    & 1 & 1 & \checkmark \\
13 & 2 & ghost    & 1 & 1 & \checkmark \\
...
For ghost shells ($|B| \le 2$): $\mathrm{struct}(1) = 2$,
$\mathrm{phantom}(1) = n_{\mathrm{base}} - 2$.
For non-ghost shells ($|B| \ge 3$): $\mathrm{struct}(1) = 0$,
...
T0 (forbidden trans.) & quasi-universal & any sieve with step~3 \\
...
\midrule
T0 (forbidden trans.\ mod~3) & YES (exact) & YES (exact) \\
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{No new physics.}  The sieve structure is exhausted at cycle~1.
    Ghost primes ($p \ge 11$) add only perturbative corrections (ghost VP),
...
\item \textbf{AT formula} (three regimes): ghost ($|B| \leq 2$, AT $= 1$),
