...
Each theorem carries a marker indicating its formalisation status in
the \texttt{PT\_LEAN} library (Lean~4\,/\,Mathlib). The legend:
\leanLegend{}
...
\medskip
\noindent\emph{Formal verification.}
Theorems T1, T3, $s=1/2$ and T2 are formally verified in Lean~4 + Mathlib in
the \texttt{PT\_LEAN/} package (540 lines, 0~\texttt{sorry},
3323~jobs \texttt{lake build} clean on Lean \texttt{v4.30.0-rc2} and
Mathlib \texttt{master @ e12bcd0}). Theorem T1 above corresponds to the
statement \texttt{T1\_forbidden\_self\_transition} in
\texttt{PT/Sieve/T1ForbiddenTransitions.lean}; its formal proof uses the
lemmas \texttt{nextSixRough\_of\_mod6\_one} and \texttt{\_five} (case
exhaustion on the residue modulo 6, then \texttt{omega}).
...
\begin{remark}[PT usage of T1; relation to Lemke Oliver--Soundararajan 2016]
\label{rem:T0_scope}
In the PT framework, T1 fixes the transition matrix at the sieve level:
$T_3 = \mathrm{antidiag}(1,1)$, \emph{exact} for $6$-rough integers
(by the elementary arithmetic argument of the proof above).
Restricted to consecutive primes (a strict sub-sequence of the
$6$-rough integers), the empirical matrix $T_3^{(p)}$ acquires small
diagonal entries: this is precisely the bias predicted by Lemke Oliver
and Soundararajan~\cite{LemkeOliverSoundararajan2016} from the
Hardy--Littlewood conjecture, with leading term
\begin{equation}
  ...
  \label{eq:LOS_diag}
\end{equation}
and symmetrically for $(2,2)$; the diagonal transitions are therefore
\emph{suppressed} relative to the antidiagonal ones by a secondary
term $(\log\log N)/(\log N)$ vanishing as $N \to \infty$. This
prime-level structure qualitatively reinforces T1 without making it
exact: it is an asymptotic convergence to the antidiagonal matrix,
not its identity.

All PT derivations (symmetry parameter $s = 1/2$, conservation theorem,
etc.) rely on the exact \emph{sieve-level} matrix ($6$-rough integers),
not on the empirical prime-level matrix: this is the layer at which
the structure is rigorous. The departure from the ideal at prime level
decreases as $N$ grows, in line with Elliott--Halberstam and Lemke
Oliver--Soundararajan.
\end{remark}
...
$\beta = 1/\mustar$ is \emph{posited}, not derived: it is the sieve
scale $T_{\rm sieve} = 1/\mustar$, taken as a primitive of the edge
sector. For the geometric distribution with parameter $q$, the
Gibbs-weighted normalization condition is:
\[
  \mathbb{E}[e^{-\beta g}] = \frac{(1-q)e^{-\beta}}{1-q\,e^{-\beta}},
\]
whose edge-chain resummation
$\sum_{n\geq 1}\mathbb{E}^{\,n} = \mathbb{E}/(1-\mathbb{E})$ equals $q$
exactly when $q = e^{-\beta}$ (a linear equation, hence a unique root).
This fixes the \emph{relation} $q \leftrightarrow \beta$ and holds for
\emph{every} $\beta$; it does not select the value $\beta = 1/\mustar$.
...
\begin{remark}[Status of the Gibbs parameter: posited]
\label{rem:gibbs_derivation}
No condition in this work forces $\beta = 1/\mustar$. Earlier versions
of this remark claimed it as the unique inverse temperature satisfying
$\partial F/\partial\beta = 0$ with $F = -\beta^{-1}\ln Z$; that claim
is withdrawn. For any partition function one has the identity
$\partial F/\partial\beta = S(\beta)/\beta^{2}$, so
$\partial F/\partial\beta = 0$ is exactly $S(\beta) = 0$---zero
entropy, approached only as $\beta \to \infty$. The condition has no
finite root, and does not involve $\mustar$.

More strongly, \emph{no algebraic condition can select} $\qminus$. By
Lindemann--Weierstrass $e^{-1/\mustar}$ is transcendental, whereas the
natural constraints on the geometric law---its moments, cumulants, and
partition function---are rational in $q$ and hence have algebraic
roots: $\langle k\rangle = \mustar/2$ gives $13/15 = \qplus$, and
$\langle k\rangle = \mustar$ gives $14/15$. Reaching a transcendental
$q$ requires introducing $\beta$ by hand, so the shared input of the
three routes below is structural, not editorial.

$\beta = 1/\mustar$ is therefore a \emph{posited primitive} of the edge
sector, empirically validated to $\sim 10^{-4}$ against 39 PDG
observables.
\end{remark}
...
\paragraph{The logarithmic spiral: multiplicative counterpart.}
The toric spiral encodes the \emph{additive} structure of the sieve
(residue classes). Its \emph{multiplicative} counterpart is the
logarithmic spiral $r = e^{\theta}$, which places integer~$n$ at
$\theta = \ln n$, $r = n$. Under the log-polar transformation
$(r, \theta) \mapsto (\ln r, \theta)$, the logarithmic spiral becomes
the Archimedean spiral $r' = \theta$ (\cref{fig:log_spiral_m1_en}):
this is the \emph{log commutation} made geometry. The same operation
turns the PT coupling $\alpha = \prod_p \sin^2 \theta_p$ into the
Regge action $S = -\sum_p \ln \sin^2 \theta_p$. At large scale,
primes distribute nearly uniformly along the spiral---the prime
number theorem visualized by arc-length equidistribution.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\linewidth]{figures/log_white_PT_500k.png}
\caption{Logarithmic spiral $r = e^{\theta}$ in log-polar form:
$5 \times 10^5$ primes, $\sim 2.52$ turns. Dashed circles are the
decades $n = 10, 100, \ldots, 10^6$ (equally spaced in $\ln n$).
Active primes $\{3, 5, 7\}$ and $\mu^{\star} = 15$ are at the
center. Full gallery in \texttt{PT\_PROJECTS/PT\_SPIRALS}.}
\label{fig:log_spiral_m1_en}
\end{figure}
