...
$(N_c - 3)(N_c + 1) = 0$.  Echo primes ($p \geq 11$) contribute
...
\epigraph{Geometry is the branch of mathematics which studies the properties
of figures. Information geometry is the branch which studies
the properties of statistical manifolds.}{Shun-ichi Amari}
...
(echo primes): they generate negligible holonomy and appear only as
...
\begin{tikzpicture}[scale=5.5, >=stealth]
  ...
  \node[right, font=\scriptsize, gray] at (0.20, 0.06) {$11$ (echo)};
  \fill[gray] (0.0650, 0.1257) circle (0.35pt);
  \draw[gray, thin] (0.0650, 0.1257) -- (0.20, 0.00);
  \node[right, font=\scriptsize, gray] at (0.20, 0.00) {$13$ (echo)};
  % annotation
...
all primes $p \geq 11$ are inactive (echo).
...
\item \textbf{Spectral interpretation.}
  ...
  and contributes only perturbatively (echo VP).
...
coupling strength; the threshold separating active from echo primes
...
\begin{tikzpicture}[scale=5.0, >=stealth]
  ...
  \node[right, font=\scriptsize, gray] at (0.58, 0.22) {$p\!=\!11$ (echo)};
  \fill[gray] ({cos(20.8)},{sin(20.8)}) circle (0.4pt);
  \draw[gray, thin] ({cos(20.8)},{sin(20.8)}) -- (0.58, 0.10);
  \node[right, font=\scriptsize, gray] at (0.58, 0.10) {$p\!=\!13$ (echo)};
  % annotation
...
\caption{Holonomy angles on the unit circle.
  ...
  Echo primes ($p \geq 11$, gray) lie closer to the axis.}
...
\begin{tikzpicture}[xscale=2.0, yscale=4.5, >=stealth]
  ...
  \node[font=\small, gray] at (4.5,0.88) {\textbf{Echo} ($\gamma_p < s$)};
  % y-axis ticks
...
\caption{Anomalous dimensions $\gamma_p$ at $\mustar = 15$.
  ...
  the three active primes $\{3,5,7\}$ (blue) from the echo
...
% ============================================================
\subsection{Echo primes and perturbative corrections}
\label{sec:ghost}

The inactive primes $\{11, 13, \ldots\}$ are not simply absent.
They appear as \emph{perturbative corrections} (echo vacuum polarization).

\begin{definition}[Echo VP weight]
\label{def:ghost_vp}
The echo vacuum polarization weight is:
\begin{equation}
  \beta_{\rm echo} = \sum_{p \in \{11,13\}} \sinsp{p}(\qrel) \cdot \gamma_p
  \approx 0.104.
  \label{eq:ghost_vp:ch06}
\end{equation}
\end{definition}

The echo VP contribution to $\alpha_{\rm EM}$ is $\delta\alpha/\alpha
= -\gamma_3 \cdot \alpha_{\rm bare} \cdot \beta_{\rm echo} \approx 14\;{\rm ppb}$
(see~\cite{companion_physics}).
...
Identification (THM, four unicities):
  ...
  $\{11,13\}$ = echo VP counter-terms (\cite{companion_physics}, DER-PHYS).
...
Validation:
  ...
  Echo VP 14\,ppb verified (\cite{companion_physics}).
\end{ledgerbox}
...
The echo primes ($p \geq 11$) contribute perturbatively as vacuum
polarization corrections.  Whether the echo VP series converges
